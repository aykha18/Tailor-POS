...
function initializeBillingSystem() {
  ...
  function updateTotals() {
    ...
    if (bill.length > 0) {
      ...
      if (printBtn) {
        ...
        printBtn.classList.remove('opacity-50', 'pointer-events-none');
      }
    } else {
      ...
      if (printBtn) {
        ...
        printBtn.classList.add('opacity-50', 'pointer-events-none');
      }
    }
  ...
  function setupProductQuickAddForInput(productInput, rateInput, formType) {
    ...
    function renderDropdownOptions(filteredProducts) {
      ...
      if (!productDropdown) return; // Double check after creation
    ...
    function hideDropdown() {
      if (isHidingDropdown || !productDropdown || !productDropdown.parentNode) {
        return;
      }
  ...
  function setupMasterAutocomplete() {const masterInput = document.getElementById('masterName');
    const masterInputMobile = document.getElementById('masterNameMobile');if (!masterInput && !masterInputMobile) {return;
    }
    ...
    // Create dropdown container
    function createMasterDropdown() {masterDropdown = document.createElement('div');
    ...
    function renderDropdownOptions(filteredEmployees) {
      ...
      options.forEach(option => {
        option.addEventListener('click', function(e) {
          ...
          window.selectedMasterId = masterId;// Update both inputs if they exist
          if (masterInput) {
  ...
  window.testMasterDropdown = function() {if (masterInputMobile) {masterInputMobile.focus();
  ...
  window.testMasterSelection = function() {
    ...
    if (masterNameElement) {
      console.log('Desktop master name:', masterNameElement.value);
    }
    
    if (masterNameMobileElement) {
      console.log('Mobile master name:', masterNameMobileElement.value);
    }
  ...
  async function handleAddItem(formType) {// Get form elements based on form type
    ...
    if (!selectedProductData && productInput?.value.trim()) {
      ...
      if (productName) {
        ...
        if (foundProduct) {
          ...
          productInput.setAttribute('data-selected-product', JSON.stringify(foundProduct));} else {
    ...
    if (existingItemIndex !== -1) {
      ...
      const newVatAmount = Math.round(newTotal * (vatPercent / 100) * 100) / 100;      // Show modern confirmation dialog
      let confirmed;
  ...
  function setupSearchAndReprint() {
    ...
    document.getElementById('searchInvoiceResults')?.addEventListener('click', async function(e) {
      ...
      if (reprintBtn) {
        ...
        // Store the current bill ID for WhatsApp functionality
  ...
  function setupPrintButton() {
    const printBtn = document.getElementById('printBtn');
    if (printBtn) {
      printBtn.addEventListener('click', async function() {if (bill.length === 0) {
        ...
        if (!selectedMasterElement || !selectedMasterElement.getAttribute('data-selected-master')) {
          ...
          try {
            ...
            masterId = selectedMaster.master_id;} catch (e) {
            console.warn('Failed to parse selected master data:', e);
          }
        } else {// Try to use global selectedMasterId as fallback
  ...
  setupPrintButton(); // Add print button setup// Make functions globally available
  ...
  window.resetBillingForm = async function() {
    ...
    // Clear current bill ID for WhatsApp functionality
  ...
  window.editBillItem = function(index) {
    ...
    if (billQuantityMobileElement) {
      billQuantityMobileElement.value = item.quantity;} else {}
    if (billPriceMobileElement) {
      billPriceMobileElement.value = item.rate; // Changed from item.price to item.rate} else {}
  ...
  // WhatsApp functionality (now includes PDF)
  ...
  async function handleWhatsAppClick() {
    ...
    try {
      ...
      if (saveResult.bill_id) {
        ...
        // Use data from billData instead of reading from potentially cleared DOM elements
